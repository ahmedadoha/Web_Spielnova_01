-- Migration: update pending_payment expiry to 32 minutes
--
-- All expiry windows are now aligned to 32 min:
--   - Stripe session expires_at  = now + 32 min  (30-min minimum + 2-min buffer)
--   - Lazy expiry in code        = 32 min
--   - Cron job                   = 32 min
--   - This RPC function          = 32 min  (updated below)
--
-- The slot and the Stripe session die at the same moment, so a customer can
-- never pay into an already-freed slot.
--
...
BEGIN
    ...
    -- pending_payment rows older than 32 min are treated as expired.
    -- 32 min = Stripe session expires_at (30-min minimum + 2-min buffer).
    v_cutoff := NOW() - INTERVAL '32 minutes';
