...
--
-- Solves the double-booking race condition where two customers submitting
-- at the same millisecond could both pass the availability check before
-- either booking is written to the database.
--
-- The fix uses pg_advisory_xact_lock() to acquire a transaction-scoped
-- advisory lock keyed on the slot start time. Only one concurrent call
-- for the same start time can hold this lock at once — all others wait.
-- The availability re-check and INSERT happen inside the same locked
-- transaction, making the entire operation atomic.
--
...
BEGIN
    ...
    -- Any two calls with the same start time compete for this lock.
    -- The lock is automatically released when the transaction ends.
    PERFORM pg_advisory_xact_lock(
        EXTRACT(EPOCH FROM DATE_TRUNC('minute', p_start_time))::BIGINT
    );

    -- pending_payment rows older than 15 min are treated as expired (lazy expiry)
