-- Migration: exclude refunded bookings from availability checks in RPC
--
-- This ensures that when a booking is refunded by staff, the slots are
-- immediately freed up in the database transaction level when creating new bookings.
--
...
BEGIN
    ...
    WHILE v_chunk_start < p_end_time LOOP
        ...
        FROM bookings
        WHERE status NOT IN ('cancelled', 'deleted', 'refunded')
